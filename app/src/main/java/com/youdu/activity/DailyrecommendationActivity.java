...
public class DailyrecommendationActivity extends BaseActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dailyrecommendation_layout);
        init();
...
//        recyclerView.setAdapter(adapter);
    ...
    private void requestRecommandData() {
        RequestCenter.requestHotvegetableData(new DisposeDataListener() {
            @Override
    ...
    private void showSuccessView() {
        if (mRecommandData.result != null && mRecommandData.result.size() > 0) {//判断数据是否为空，不为空则服务器为我们返回了数据，真正去调用
            mAdapter = new HotvegetableAdapter(DailyrecommendationActivity.this, mRecommandData.result);//mRecommandData.data.list为我们的数据，mcontext为上下文，courseadapter为我们的每个ui更新
            ...
            mListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    Intent intent = new Intent(DailyrecommendationActivity.this, ButtonActivity.class);
...
//                    startActivity(intent);
                    ...
                    intent.putExtra(ButtonActivity.IMG1,value.step.get(0).stepImg);
                    intent.putExtra(ButtonActivity.IMG2,value.step.get(1).stepImg);
                    intent.putExtra(ButtonActivity.IMG3,value.step.get(2).stepImg);
                    intent.putExtra(ButtonActivity.IMG4,value.step.get(3).stepImg);
                    intent.putExtra(ButtonActivity.TEXT1,value.step.get(0).stepName);
                    intent.putExtra(ButtonActivity.TEXT2,value.step.get(1).stepName);
                    intent.putExtra(ButtonActivity.TEXT3,value.step.get(2).stepName);
                    intent.putExtra(ButtonActivity.TEXT4,value.step.get(3).stepName);
